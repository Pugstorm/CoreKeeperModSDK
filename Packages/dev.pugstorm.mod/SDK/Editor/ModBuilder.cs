	...
	public static class ModBuilder
	{
		...
		public static void BuildMod(ModBuilderSettings settings, string exportPath, Action<bool> callback, bool installInSubDirectory = true)
		{
			...
			try
			{
				...
				List<IncludedFile> manifest = new();
				...
				if (settings.buildBundles)
				{
					...
					else
					{
						...
						{
							manifest.Add(new IncludedFile(file, null));
						}
					}
				}
				...
				{
					FileInfo fileInfo = new FileInfo(outputFile.path);
					...
					modManifest.files.Add(new ModFile { path = path, guid = outputFile.guid });
				}
		...
		private static void BuildConf(string modDirectory, string outputPath, List<string> assetPaths, List<IncludedFile> manifest)
		{
		...
		private static void BuildLocalization(string modDirectory, string outputPath, List<string> assetPaths, List<IncludedFile> manifest)
		{
		...
		private static void BuildScripts(string modDirectory, string modName, string outputPath, List<string> assetPaths, List<IncludedFile> manifest,
			bool forceReimport)
		{
			DirectoryInfo directoryInfo = new(modDirectory);
			List<string> scriptPaths = new();
			List<string> relativeDestPaths = new();
			int numberNonGeneratedScripts = 0;
			...
			for (int i = 0; i < scriptPaths.Count; ++i)
			{
				...
				string guid = i < numberNonGeneratedScripts ? AssetDatabase.AssetPathToGUID(scriptPaths[i]) : null;
				manifest.Add(new IncludedFile(destFileInfo.FullName, guid));
			}
		...
		private static void BuildLibraries(string modDirectory, string outputPath, List<string> assetPaths, List<IncludedFile> manifest)
		{
			...
			foreach (var dll in dllPaths)
			{
				...
				File.Copy(fileInfo.FullName, destPath, true);
				manifest.Add(new IncludedFile(destPath, AssetDatabase.AssetPathToGUID(dll)));
			}
		}
			...
			List<IncludedFile> manifest)
		{
			...
			for (int i = assetPaths.Count - 1; i >= 0; --i)
			{
				...
				assetGuids.Add(AssetDatabase.AssetPathToGUID(asset));
				assetPaths.RemoveAt(i);
			}
			...
			foreach (var buildConfig in buildConfigs)
			{
				...
				PugMod.AssetBundleManifest assetBundleManifest = new() { assets = new() };
				for (int i = 0; i < assetsToIncludeInBundle.Count; ++i)
				{
					assetBundleManifest.assets.Add(new ModFile
					{
						path = assetsToIncludeInBundle[i],
						guid = assetGuids[i],
					});
				}
				string assetBundleManifestJson = JsonUtility.ToJson(assetBundleManifest, true);
				...
				if (bundleBuilds.Count != 0)
				{
					...
					{
						manifest.Add(new IncludedFile(outputBundle.Value.FileName, null));
						var assetBundleManifestPath = outputBundle.Value.FileName + ".manifest";
						File.WriteAllText(Path.Combine(buildParams.OutputFolder, assetBundleManifestPath), assetBundleManifestJson);
						manifest.Add(new IncludedFile(assetBundleManifestPath, null));
					}
				}
			}
		...
		private static void IncludeAssetDirectly(string assetPath, string modDirectory, string installDirectory, List<IncludedFile> manifest)
		{
			...
			manifest.Add(new IncludedFile(destPath, AssetDatabase.AssetPathToGUID(assetPath)));
		}
		...
		{
			public IncludedFile(string path, string guid)
			{
				this.path = path;
				this.guid = guid;
			}

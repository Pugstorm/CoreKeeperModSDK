	...
	public partial class ModSDKWindow
	{
		private class CreateMod
		{
			...
			public void OnEnable(VisualElement root)
			{
				...
				VisualTreeAsset modDependencyEntryTemplate = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>("Packages/dev.pugstorm.mod/Assets/UI/ModDependencyEntry.uxml");
				...
				_buildModButton.clicked += () =>
				{
					...
					string tempFolderPath = Path.Combine(Path.GetTempPath(), "BuiltMods");

					ModBuilder.BuildMod(modSettings, tempFolderPath, success =>
					{
						if (!success)
						{
							ShowError("Failed to build mod");
							return;
						}

						EditorUtility.DisplayDialog("Build", $"Mod {modSettings.metadata.name} successfully built at {tempFolderPath}", "OK");

						_lastBuildPath = tempFolderPath;
						AddPath(Path.Combine(_lastBuildPath, modSettings.metadata.name));
					});
				};
				...
				_installModButton.clicked += () =>
				{
					...
					string installModPath = Path.Combine(path, modSettings.metadata.name);//temp folder path
					Directory.CreateDirectory(installModPath);
					string builtModPath = Path.Combine(Path.GetTempPath(), "BuiltMods", modSettings.metadata.name);

					foreach (string dir in Directory.GetDirectories(builtModPath, "*", SearchOption.AllDirectories))//gets folders and contents
					{
						Directory.CreateDirectory(dir.Replace(builtModPath, installModPath));
					}

					foreach (string file in Directory.GetFiles(builtModPath, "*", SearchOption.AllDirectories))//gets modmanifest.json
					{
						string destFile = file.Replace(builtModPath, installModPath);
						File.Copy(file, destFile, true);
					}

					EditorUtility.DisplayDialog("Install", $"Mod {modSettings.metadata.name} successfully installed to {installModPath}", "OK");
				...
				_openLastBuildFolder.clicked += () =>
				{
					...
					{
						ShowError("No recent build folder found. Please build your mod first.");
						return;
					}
			...
			private void AddPath(string newPath)
			{
				...
				if (!modPaths.latestBuildOrInstallPaths.Contains(newPath))
				{
					modPaths.latestBuildOrInstallPaths.Add(newPath);
					if (modPaths.latestBuildOrInstallPaths.Count > 5)
					{
						modPaths.latestBuildOrInstallPaths.RemoveAt(0);
					}

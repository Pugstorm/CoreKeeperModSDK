			...
			try
			{
				...
				if (File.Exists(SDK_ZIP_PATH))
				{
					...
					foreach (var zipFile in zip.Entries)
					{
						...
						{
							if (!Directory.Exists(settings.gameAssemblyPath))
							{
								Directory.CreateDirectory(settings.gameAssemblyPath);
							}
							
							// Found matching editor assembly, install this
							zipFile.ExtractToFile(Path.Combine(settings.gameAssemblyPath, zipFile.Name), true);
						}
					}
				}
			}

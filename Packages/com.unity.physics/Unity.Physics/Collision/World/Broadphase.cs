...
namespace Unity.Physics
{
    ...
    internal struct Broadphase : IDisposable
    {
        ...
        public int NumStaticBodiesCapacity => m_StaticTree.NumBodiesCapacity;

        public int NumDynamicBodies => m_DynamicTree.NumBodies;
        public int NumDynamicBodiesCapacity => m_DynamicTree.NumBodiesCapacity;
        ...
        public void CopyFrom(Broadphase broadphase)
        {
            m_StaticTree.CopyFrom(broadphase.m_StaticTree);
            m_DynamicTree.CopyFrom(broadphase.m_DynamicTree);
        }
        ...
        {
            public NativeArray<Node> Nodes => m_Nodes.GetSubArray(0, NumNodes);
            public NativeArray<CollisionFilter> NodeFilters => m_NodeFilters.GetSubArray(0, NumNodes);
            public NativeArray<CollisionFilter> BodyFilters => m_BodyFilters.GetSubArray(0, NumBodies);
            public NativeArray<bool> RespondsToCollision => m_RespondsToCollision.GetSubArray(0, NumBodies);
            
            [NoAlias] private NativeArray<Node> m_Nodes; // The nodes of the bounding volume
            [NoAlias] private NativeArray<CollisionFilter> m_NodeFilters; // The collision filter for each node (a union of all its children)
            [NoAlias] private NativeArray<CollisionFilter> m_BodyFilters; // A copy of the collision filter of each body
            [NoAlias] private NativeArray<bool> m_RespondsToCollision; // A copy of the RespondsToCollision flag of each body
            ...
            [NoAlias] internal NativeArray<int> Lengths;

            internal Allocator Allocator;

            public BoundingVolumeHierarchy BoundingVolumeHierarchy => new BoundingVolumeHierarchy(m_Nodes, m_NodeFilters);

            public int NumBodiesCapacity => m_BodyFilters.Length;
            public int NumBodies => Lengths[0];
            public int NumNodes => Lengths[0] + BoundingVolumeHierarchy.Constants.MaxNumTreeBranches;

            public Tree(int numBodies, Allocator allocator = Allocator.Persistent)
            {
                this = default;
                Allocator = allocator;
                Lengths = new NativeArray<int>(1, allocator, NativeArrayOptions.ClearMemory);
                Reset(numBodies);
            ...
            public void Reset(int numBodies)
            {
                ...
                if (numNodes > m_Nodes.Length)
                {
                    var ceilPow2Bodies = math.ceilpow2(numBodies);
                    numNodes = ceilPow2Bodies + BoundingVolumeHierarchy.Constants.MaxNumTreeBranches;
                    SetCapacity(ceilPow2Bodies, numNodes);
                }

                Lengths[0] = numBodies;
            }

            private void SetCapacity(int numBodies, int numNodes)
            {
                if (m_Nodes.IsCreated)
                {
                    m_Nodes.Dispose();
                }
                m_Nodes = new NativeArray<BoundingVolumeHierarchy.Node>(numNodes, Allocator, NativeArrayOptions.UninitializedMemory)
                {
                ...
                if (m_NodeFilters.IsCreated)
                {
                    m_NodeFilters.Dispose();
                }
                m_NodeFilters = new NativeArray<CollisionFilter>(numNodes, Allocator, NativeArrayOptions.UninitializedMemory)
                {
                ...
                m_BodyFilters = new NativeArray<CollisionFilter>(numBodies, Allocator, NativeArrayOptions.UninitializedMemory);

                if (m_RespondsToCollision.IsCreated)
                {
                    m_RespondsToCollision.Dispose();
                }
                m_RespondsToCollision = new NativeArray<bool>(numBodies, Allocator, NativeArrayOptions.UninitializedMemory);
            }
            ...
            public Tree Clone()
            {
                ...
                {
                    Allocator = Allocator,
                    m_Nodes = new NativeArray<BoundingVolumeHierarchy.Node>(m_Nodes, Allocator),
                    m_NodeFilters = new NativeArray<CollisionFilter>(m_NodeFilters, Allocator),
                    m_BodyFilters = new NativeArray<CollisionFilter>(m_BodyFilters, Allocator),
                    m_RespondsToCollision = new NativeArray<bool>(m_RespondsToCollision, Allocator),
                    ...
                    Lengths = new NativeArray<int>(Lengths, Allocator),
                };
            }

            public void CopyFrom(Tree tree)
            {
                Reset(tree.NumBodies);
                Nodes.CopyFrom(tree.Nodes);
                NodeFilters.CopyFrom(tree.NodeFilters);
                BodyFilters.CopyFrom(tree.BodyFilters);
                RespondsToCollision.CopyFrom(tree.RespondsToCollision);
                Ranges.CopyFrom(tree.Ranges);
                BranchCount.CopyFrom(tree.BranchCount);
            }
        ...
        struct BuildBroadphaseJob : IJob
        {
            ...
            public void Execute()
            {
                PhysicsWorld.CollisionWorld.Broadphase.Build(PhysicsWorld.StaticBodies, PhysicsWorld.DynamicBodies, PhysicsWorld.MotionVelocities,
                    PhysicsWorld.CollisionWorld.CollisionTolerance, TimeStep, Gravity, BuildStaticTree.Value == 1);
            }
        }
        ...
        struct PrepareDynamicBodyDataJob : IJobParallelFor
        {
            ...
            {
                ExecuteImpl(index, AabbMargin, Gravity, TimeStep, PhysicsWorld.DynamicBodies, PhysicsWorld.MotionVelocities, Aabbs, Points, FiltersOut, RespondsToCollisionOut);
            }
